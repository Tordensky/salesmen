...
{ 
    ...
    gs_clear(WHITE);
        // Plot each grain bag
    for(i = 0; i < nBags; i++)
        gs_dot(bagCoords[i].x, bagCoords[i].y, 10, RED); 
    
        // Plot edges in the path
    for(i = 0; i < nBags-1; i++)
        gs_line(bagCoords[(int)path[i  ]].x, bagCoords[(int)path[i  ]].y, 
                bagCoords[(int)path[i+1]].x, bagCoords[(int)path[i+1]].y, RED);    
    
        // Plot the final edge closing the path
    gs_line(bagCoords[(int)path[nBags-1]].x, bagCoords[(int)path[nBags-1]].y, 
            bagCoords[(int)path[0      ]].x, bagCoords[(int)path[0      ]].y, RED);
